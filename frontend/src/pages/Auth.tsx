...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    if (hash.includes("oauth_success")) {
      ...
      setErrors({
        general:
          "Social authentication failed or is unconfigured. Please try again.",
      });
    }
  }, [navigate, location]);
  ...
  const handleOAuth = async (provider: string) => {
    try {
      ...
      setErrors({
        general: `${provider.charAt(0).toUpperCase() + provider.slice(1)} login is not currently configured by the admin.`,
      });
    } finally {
      setIsLoading(false);
    }
